...
class HashTable:
    ...
    # keyとvalueを再ハッシュされたハッシュテーブルに入れるだけの関数を再定義
    def private_put(self, key, value):
        assert type(key) == str
    ...
    def make_new_buckets(self):
        ...
        for item in previous_buckets:
            while item:
                self.private_put(item.key, item.value)
                item = item.next
        return True

    def re_hash(self):
        if self.item_count > self.bucket_size * 0.7:
            self.bucket_size = 2 * self.bucket_size + 1
            print(self.bucket_size)
            self.make_new_buckets()
            return True
